...
//**************************************************************************************************
char* _dbgprint ( const char* format, ... )
{
...
bool VS1053::testComm ( const char *header )
{
  ...
  _dbgprint ( header ) ;                                 // Show a header
  if ( !digitalRead ( dreq_pin ) )
  {
    _dbgprint ( "VS1053 not properly installed!" ) ;
  ...
  for ( i = 0 ; ( i < 0xFFFF ) && ( cnt < 20 ) ; i += delta )
  {
    ...
    {
      _dbgprint ( "VS1053 SPI error. SB:%04X R1:%04X R2:%04X", i, r1, r2 ) ;
      cnt++ ;
      delay ( 10 ) ;
    }
  }
...
void VS1053::begin()
{
  ...
  if ( testComm ( "Slow SPI, Testing VS1053 read/write registers..." ) )
  {
    ...
    _dbgprint ( "endFillByte is %X", endFillByte ) ;
...
//  return okay && sdi_send_buffer ( data, len ) ;        // True if more data can be added to fifo
  sdi_send_buffer ( data, len ) ;        // True if more data can be added to fifo
}
